  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      if (otp === generatedOtp && otp.length === 6) {
        ...
        if (foundUser) {
          const success = login(foundUser.email, 'password');
          if (success) {
            navigate('/');
          } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-900">eMedica</h2>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
          <div className="flex justify-center mb-6 space-x-2">
            <button
              type="button"
              className={`px-3 py-2 rounded-lg font-medium text-sm transition-colors ${loginMethod === 'password' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            ...
            <button
              type="button"
              className={`px-3 py-2 rounded-lg font-medium text-sm transition-colors ${loginMethod === 'otp-phone' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
                ...
                <>
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 tracking-widest text-center"
                  ...
                  <button
                    ...
                    className="w-full bg-blue-600 text-white py-3 rounded-lg font-medium hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
                  >
          ...
          <div className="space-y-2">
            {mockUsers.map((cred, index) => (
              <div key={index} className="flex justify-between text-xs">
                ...
                <span className="text-gray-600">{cred.email} / {cred.phone}</span>
              </div>
            ))}
            <p className="text-xs text-gray-500 mt-2">Password for all: "password"</p>
            <p className="text-xs text-gray-500 mt-1">Phone OTP for Doctor: 9999999991<br/>Patient: 9999999992<br/>Admin: 9999999993<br/>Super Admin: 9999999994<br/>Trainee: 9999999995</p>
          </div>
        </div>
      </div>
    </div>
